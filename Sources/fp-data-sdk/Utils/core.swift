...
public class NucleusState {
    ...
    func emitEvents(event: String, eventmetadata: [String: Any]) {
        ...
        if (event == "viewBegin") {
            self.data["view_start"] = currentTime
        }
    ...
    func demolishView() {
          if self.playerDestroyed as? Bool == true {
                return
            }
        ...
        func validateData() {
            ...
            for key in numericalKeys {
                if let value = self.data[key] as? String, let intValue = Int(value) {
                    self.data[key] = intValue
                } else {
                    self.data[key] = nil
                }
            }
        ...
        func filterData(eventName: String) {
            ...
            if let playerDuration = self.data["player_source_duration"] as? Int,
               let videoDuration = self.data["video_source_duration"] as? Int {
                self.data["video_source_is_live"] = (playerDuration > 0 || videoDuration > 0) ? false : nil
